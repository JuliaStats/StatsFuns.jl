...
@testset "logmvgamma" begin
    ...
    @testset "consistent with loggamma" for eltya in (Float32, Float64)
        ...
        @test logmvgamma(3, a) ≈
              eltya((3/2)*logπ) + loggamma(a) + loggamma(a - eltya(0.5)) +
              loggamma(a - one(a))
    end
    ...
    @testset "consistent with itself" for eltya in (Float32, Float64)
        ...
        for p in 1:50
            ...
            @test logmvgamma(p, a) ≈
                  eltya((p/2-1/2)*logπ) + loggamma(a) + logmvgamma(p - 1, a - eltya(0.5))
        end
    end
end
...
@testset "gammalogcdf: numerical issue" begin
    @test gammalogcdf(42648.50647826826, 2.2498007956420723e-5, 0.6991377135675367) ≈
          -1933.2698959040617410
end
