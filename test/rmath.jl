...
function rmathcomp(basename, params, X::AbstractArray, rtol = _default_rtol(params, X))
    ...
    @testset "cdf with x=$x" for x in X
        check_rmath(cdf, stats_cdf, rmath_cdf, params, "x", x, true, rtol)
    end
    @testset "ccdf with x=$x" for x in X
        check_rmath(ccdf, stats_ccdf, rmath_ccdf, params, "x", x, true, rtol)
    end
    @testset "logcdf with x=$x" for x in X
        check_rmath(logcdf, stats_logcdf, rmath_logcdf, params, "x", x, false, rtol)
    end
    @testset "logccdf with x=$x" for x in X
        check_rmath(logccdf, stats_logccdf, rmath_logccdf, params, "x", x, false, rtol)
    end
    ...
    if test_inv
        @testset "invcdf with q=$_p" for _p in p
            check_rmath(invcdf, stats_invcdf, rmath_invcdf, params, "q", _p, false, rtol)
        end
        @testset "invccdf with q=$_p" for _p in cp
            check_rmath(invccdf, stats_invccdf, rmath_invccdf, params, "q", _p, false, rtol)
        end
...
@testset "RMath" begin
    ...
    @testset "$(f)(0, 0, $x) should error" for f in
                                               (betacdf, betaccdf, betalogcdf, betalogccdf),
        x in (0.0, 0.5, 1.0)
    ...
    rmathcomp_tests(
        ...
        [
            ...
            (
                ...
                Float16(0.01):Float16(0.01):Float16(0.99),
            ),
    ...
    rmathcomp_tests("signrank", [((4), -2:12), ((4), -2.0:0.25:12.0), ((10), -2:57)])

    # The R version does not roundtrip cdf->invcdf, while our version does
    @test_broken RFunctions.signrankinvcdf.(50, RFunctions.signrankcdf.(50, -1:1276)) ==
                 [0; 0:1275; 1275]
