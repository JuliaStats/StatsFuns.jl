...
"""
logmvbeta(p::Int, a::T, b::T) where {T<:Real} =
    logmvgamma(p, a) + logmvgamma(p, b) - logmvgamma(p, a + b)
...
const lstirlingF64 = Float64[lstirling_asym(k) for k = big(1):big(64)]
const lstirlingF32 = [Float32(lstirlingF64[i]) for i = 1:40]
