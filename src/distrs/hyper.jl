...
function _crithyperg(j::Float64, k::Float64, m::Float64, cprob::Float64)
    ...
    μ = j * k / m
    denom = m * m * max(m - 1.0, 1.0)
    σ = sqrt(j * k * (m - j) * (m - k) / denom)
    i = clamp(floor(μ + norminvcdf(cprob) * σ + 0.5), mn, mx)
    ...
    if pr >= cprob
        ...
        if pr < 1.00001 * tpr
            # PMF dominates: use ratio stepping
            tpr *= ((i + 1.0) * (m - j - k + i + 1.0)) / ((k - i) * (j - i))
            i -= 1.0
            while tpr > cprob && i >= mn
                tpr *= ((i + 1.0) * (m - j - k + i + 1.0)) / ((k - i) * (j - i))
                i -= 1.0
            end
        else
            ...
            # Step through using PMF ratio
            tpr_step = tpr * ((i + 1.0) * (m - j - k + i + 1.0)) / ((k - i) * (j - i))
            pr -= tpr_step
            while pr >= cprob && i > mn
                i -= 1.0
                tpr_step *= ((i + 1.0) * (m - j - k + i + 1.0)) / ((k - i) * (j - i))
                pr -= tpr_step
            end
        end
        return i + 1.0
