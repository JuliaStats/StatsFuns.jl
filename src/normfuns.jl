...
# invcdf
norminvcdf(p::Float64) = qnorm_kernel(p, false)
norminvcdf(p::Real) = norminvcdf(f64(p))
norminvcdf(μ::Real, σ::Real, p::Real) = xval(μ, σ, norminvcdf(p))

# invccdf
norminvccdf(p::Float64) = -norminvcdf(p)
norminvccdf(p::Real) = norminvccdf(f64(p))
norminvccdf(μ::Real, σ::Real, p::Real) = xval(μ, σ, norminvccdf(p))

# invlogcdf
norminvlogcdf(lp::Float64) = qnorm_kernel(lp, true)
norminvlogcdf(lp::Real) = norminvlogcdf(f64(lp))
norminvlogcdf(μ::Real, σ::Real, lp::Real) = xval(μ, σ, norminvlogcdf(lp))

# invlogccdf
norminvlogccdf(lp::Float64) = -norminvlogcdf(lp)
norminvlogccdf(lp::Real) = norminvlogccdf(f64(lp))
norminvlogccdf(μ::Real, σ::Real, lp::Real) = xval(μ, σ, norminvlogccdf(lp))


# norminvcdf & norminvlogcdf
#
...
#
function qnorm_kernel(arg::Float64, uselog::Bool)
    if (!uselog)
        q = arg - 0.5
    else
        q = exp(arg) - 0.5
