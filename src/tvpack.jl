...
function tvtcdf(nu::Int, h::Vector{Float64}, r::Vector{Float64})
    ...
    elseif r23 + 1.0 < eps()
        ...
        ar = asin(r23)
        ruc = pt * sign(ar) - ar
        tvt += adonet(x->tvtmfn(x, rua, rub, nu, h1, h2, h3, r23, ar, ruc), 0.0, 1.0,
                      sqrt(eps())) / (4.0*pt)
    end
...
function tvtmfn(x::Float64, rua::Float64, rub::Float64, nu::Int, h1::Float64, h2::Float64,
                h3::Float64, r23::Float64, ar::Float64, ruc::Float64)
    retval = 0.0
    r12, rr2 = sincs(rua * x)
    r13, rr3 = sincs(rub * x)
    if abs(rua) > 0
...
function pntgnd(nu::Int, ba::Float64, bb::Float64, bc::Float64, ra::Float64, rb::Float64,
                r::Float64, rr::Float64)
    retval = 0.0
...
function bvtcdf(nu::Int, dh::Float64, dk::Float64, r::Float64)
    ...
    elseif r + 1.0 < eps()
        ...
        if mod(nu, 2) == 0
            ...
            hkn = dh*dk - nu
            hpk = dh + dk
            bvt = atan2(-snu * (hkn * qhrk + hpk * hkrn), hkn * hkrn - nu * hpk * qhrk) /
                  (2.0pi)
            if bvt < -eps()
                bvt += 1.0
            end
